...
<template>
	<section
		class="section-bg space-y-6 p-3 xl:p-2 rounded-2xl transition-all duration-500 ease-in-out"
	>
		<div class="grid md:grid-cols-[1fr,300px] gap-4 sm:gap-6">
			...
			<div
				class="space-y-6 p-3 xl:p-5 rounded-2xl flex flex-col justify-end transition-all duration-500 ease-in-out"
			>
				<div
					class="relative space-y-4 sm:space-y-6 text-left p-3 xl:p-5 flex-grow"
				>
					<span class="section-title text-2xl sm:text-3xl font-bold block mb-8">
						About Me
					</span>

					<div class="space-y-4 relative">
						<div
							class="absolute left-[18px] top-[15px] w-[2px] h-[calc(100%-40px)] xl:h-[calc(105%-40px)] dark:bg-zinc-800/90 bg-gray-300/80"
						></div>
						<div class="relative pl-10 sm:pl-20">
							<div class="absolute left-[9px] top-1">
								<div class="w-5 h-5">
									<div
										class="absolute inset-0 rounded-full dark:bg-zinc-800/100 bg-gray-300/80 border-2 dark:border-transparent border-white"
									></div>
								</div>
							</div>
